...
public class StatisticiRepository {
    ...
    public List<Map<String, Object>> getEvolutieVizualizari() {
        String sql = """
                SELECT TO_CHAR(data_vizualizare, 'YYYY-MM') AS luna,
                       COUNT(*)                              AS numar_vizualizari,
                       COUNT(DISTINCT id_client)             AS clienti_unici,
                       ROUND(AVG(vot)::NUMERIC, 2)           AS rating_mediu
                FROM vizualizari
                WHERE data_vizualizare >= NOW() - INTERVAL '12 months'
                GROUP BY TO_CHAR(data_vizualizare, 'YYYY-MM')
                ORDER BY luna
                """;
        return jdbc.queryForList(sql);
    }
    ...
    public List<Map<String, Object>> getFilmeCeleMainComentate(Integer limit) {
        String sql = """
                SELECT f.titlu,
                       c.nume AS categorie,
                       COUNT(cf.id) AS total_comentarii,
                ...
                JOIN comentarii_filme cf ON cf.id_film = f.id
    ...
    public List<Map<String, Object>> getTopOptiuniBifate() {
        String sql = """
                SELECT op.eticheta,
                       op.tip,
                       COUNT(os.id) AS de_cate_ori_bifata
                FROM optiuni_predefinite op
                LEFT JOIN optiuni_selectate os ON os.id_optiune = op.id
                GROUP BY op.id, op.eticheta, op.tip
