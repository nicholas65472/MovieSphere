...
public class CategorieRepository {
    ...
    public List<Map<String, Object>> getAllCategorii() {
        String sql = """
                SELECT c.id, c.nume, c.descriere,
                       COUNT(f.id) AS numar_filme
                FROM categorii c
    ...
    public List<Map<String, Object>> getFilmeByCategorie(Integer idCategorie) {
        String sql = """
                SELECT f.id, f.titlu, f.descriere, f.data_lansarii,
                       f.durata_minute, f.rating, f.numar_voturi, f.poster_url
                FROM filme f
                WHERE f.id_categorie = ? AND f.activ = TRUE
    ...
    public List<Map<String, Object>> getStatisticiCategorii() {
        String sql = """
                SELECT c.nume AS categorie,
                       COUNT(DISTINCT f.id)  AS numar_filme,
                       COUNT(v.id)           AS total_vizualizari,
                       ROUND(AVG(v.vot)::NUMERIC, 2) AS rating_mediu
                FROM categorii c
                LEFT JOIN filme f ON f.id_categorie = c.id AND f.activ = TRUE
                LEFT JOIN vizualizari v ON v.id_film = f.id
